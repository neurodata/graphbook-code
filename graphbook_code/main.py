...
def heatmap(
    ...
    **kwargs : dict, optional
    ...
    elif not isinstance(xticklabels, (bool, list, int)):
    ...
    if isinstance(yticklabels, list):
        if len(yticklabels) != X.shape[0]:
            ...
            raise ValueError(msg)
    elif not isinstance(yticklabels, (bool, list, int)):
        msg = "yticklabels must be a bool, list, or int, not {}".format(
            type(yticklabels)
        )
        raise TypeError(msg)
...
def plot_network(network, labels, *args, **kwargs):
    ...
    adjplot(network, plot_type="scattermap")
